...
test.describe('Sprint 2 - Core Pages', () => {
  test('home page renders instant chart form before SEO links', async ({ page }) => {
    ...
    await expect(page.locator('#home-birth-hour')).toContainText('Tý sớm (00:00-00:59)')
    await expect(page.locator('#home-birth-hour')).toContainText('Tý muộn (23:00-23:59)')
    await expect(form).not.toContainText('Tử sớm')
    await expect(form).not.toContainText('Tử muộn')
    await expect(form).not.toContainText('Tý (23h-01h)')
  ...
  test('lap-la-so page renders approved real Tu Vi landing content', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /Tiếp tục/i }).last().click()
    await expect(page.locator('main')).toContainText('Tý sớm (00:00-00:59)')
    await expect(page.locator('main')).toContainText('Tý muộn (23:00-23:59)')
    await expect(page.locator('main')).toContainText('Tý sớm (00:00-00:59) giữ nguyên ngày âm lịch')
    await expect(page.locator('main')).toContainText('Tý muộn (23:00-23:59) tính sang ngày âm lịch hôm sau')
    await expect(page.locator('main')).not.toContainText('Tử sớm')
    await expect(page.locator('main')).not.toContainText('Tử muộn')
    await expect(page.locator('main')).not.toContainText('0h-1h')
    await expect(page.locator('main')).not.toContainText('23h-24h')
  })
})
